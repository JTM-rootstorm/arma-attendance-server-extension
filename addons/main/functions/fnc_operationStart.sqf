...
if (missionNamespace getVariable ["AASE_operationActive", false]) exitWith {
    [format ["Operation start skipped because an operation is already active. source=%1", _sourceKind], "WARN"] call AASE_fnc_log;
    ""
};

private _payload = [_sourceKind, _sourceMeta] call AASE_fnc_buildOperationStartPayload;
...
if (_operationId isNotEqualTo "") then {
    ...
    [format ["Operation started: %1 source=%2", _operationId, _sourceKind], "INFO"] call AASE_fnc_log;
} else {
