...
if (_operationId isEqualTo "") exitWith {
    [format ["Operation finish skipped because no operation is active. source=%1", _sourceKind], "WARN"] call AASE_fnc_log;
    ""
};

[] call AASE_fnc_presenceFinalizeForEnd;

private _payload = [_operationId, _sourceKind, _sourceMeta] call AASE_fnc_buildOperationFinishPayload;
...
if (_accepted) then {
    ...
    [format ["Operation finished: %1 source=%2", _operationId, _sourceKind], "INFO"] call AASE_fnc_log;
} else {
