...
if ([_unit] call TCWA3_fnc_isHeadlessClient) exitWith {false};
if (!isPlayer _unit) exitWith {false};

private _uid = getPlayerUID _unit;
if (_uid isEqualTo "") exitWith {false};

private _playerName = [name _unit] call TCWA3_fnc_sanitizePlayerName;
private _scores = [getPlayerScores _unit] call TCWA3_fnc_scoreNormalizeArray;
